...
async function insertLeaguesInMongoDb(db) {
    ...
    await Promise.all(arraySeason.map(async (element) => {
        return new Promise((resolve, reject) => {
            const url = `https://api.sportmonks.com/v3/football/leagues/${element}?api_token=${TOKEN}&include=country;latest.scores;latest.participants;currentSeason&timezone=Europe/Rome`;
